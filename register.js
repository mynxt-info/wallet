    ...
    $("#register").click(function () {
        ...
        if (error) {
            ...
            if(typeof wallet_data.encrypted_wallet === "undefined") {
                wallet_data = generateWallet(master_password); // generate a wallet encrypted by the master password
            }


            wallet_data.recaptcha_challenge_field = $("#recaptcha_challenge_field").val();
            wallet_data.recaptcha_response_field = $("#recaptcha_response_field").val();
            wallet_data.tx_login_password = $("#tx_login_password").val();
            wallet_data.tx_login_password2 = $("#tx_login_password2").val();
            wallet_data.tx_email = $("#tx_email").val();

            $.post('api/register.php', wallet_data, function (data) {
                console.log(data);
